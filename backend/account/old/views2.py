...
def downloadResume(request):
    ...
    result, success = user_service.download_resume(user)

    if success:
        logger.info('Resume downloaded for user: %s', user.username)
        return result
    else:
        logger.error('Error occurred while downloading resume: %s', result['error'])
